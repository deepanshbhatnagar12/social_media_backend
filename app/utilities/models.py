...
class Address(models.Model):
    ...
    def __str__(self):
        if self.address2 is None:
            ...
            return "{}, {}, {}, {} - {}".format(self.address1, self.address2, self.city, self.state, self.pincode)
...
class BaseModelWithCreatedInfoUUID(BaseModelWithCreatedInfo):
    id = ShortUUIDField(primary_key=True, auto=False, editable=False)
    ...
    def save(self, **kwargs):
        # used only for objects created by web (not api or through scripts)
        if not self.id:
            self.id = six.text_type(shortuuid.ShortUUID().random(length=22))

            if get_request() is not None and get_request().user.is_authenticated:
                self.created_by = get_request().user

        super(BaseModelWithCreatedInfoUUID, self).save(**kwargs)
...
def get_notes_file_path(instance, filename):
    return "chapters/{0}/notes/{1}".format(instance.name, filename)
...
def get_document_path(instance, filename):
    if instance.content_type and instance.document:
        return "{}/{}/{}".format(instance.content_type.model, instance.document.object_id(), filename)
    return "general/{}_{}".format(instance.version, filename)
...
class BaseImageModel(models.Model):
    image_file = models.ImageField(_("Image"), upload_to=get_image_path, null=True, blank=True)
...
class Document(BaseModelWithCreatedInfo, BaseImageModel):
...
class ProjectConfigurations(models.Model):
    ...
    type = models.CharField(
        max_length=2,
        choices=(("S", "string"), ("I", "int"), ("D", "decimal"), ("C", "csv")),
        default="S",
    )
    ...
    def clean(self):
        if self.type == "I":
            try:
                ...
                raise ValidationError({"_value": _("Value {} is not a valid Integer .".format(self._value))})
        if self.type == "D":
            try:
                Decimal(self._value)
                ...
                raise ValidationError(
                    {
                        "_value": [_("Value {} is not a valid Decimal .".format(self._value))],
                    }
                )
