...
from simple_history.admin import SIMPLE_HISTORY_EDIT, SimpleHistoryAdmin
...
class SimpleHistoryAdminWithInline(SimpleHistoryAdmin):
    def history_form_view(self, request, object_id, version_id, extra_context=None):
        ...
        historical_obj = get_object_or_404(model, **{original_opts.pk.attname: object_id, "history_id": version_id})
        ...
        for FormSet, inline in self.get_admin_formsets_with_inline(*[request], obj):
            ...
            qs = inline.model.history.filter(
                **{
                    fk.name: obj,
                    "history_date__lte": adjusted_historical_date,
                    "history_type__in": ["+", "~"],
                }
            )
        ...
        return super(SimpleHistoryAdminWithInline, self).history_form_view(
            request, object_id, version_id, extra_context=extra_context_new
        )
